...
class SupplierClassification(Base):
    ...
    run_id = Column(String(36), nullable=True, index=True)  # UUID format (nullable for migration compatibility, but always provided)
    ...
    l1_category = Column(String(100), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        Index("idx_run_supplier_l1", "run_id", "supplier_name", "l1_category"),
        Index("idx_supplier_hash", "supplier_name", "transaction_hash"),  # Backward compatibility
        Index("idx_supplier_l1", "supplier_name", "l1_category"),  # Backward compatibility
    )
